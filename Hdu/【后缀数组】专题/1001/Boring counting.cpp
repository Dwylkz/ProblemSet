...
void solve()
{
    ...
    Rep(x, 1, l / 2 + 1)
    {
        ...
        Rep(j, 0, l)
        {
            ...
            {
                if (zkl.sa[i] > maxi) maxi = zkl.sa[i];
                if (zkl.sa[i] < mini) mini = zkl.sa[i];
                if (zkl.sa[i - 1] > maxi) maxi = zkl.sa[i - 1];
                if (zkl.sa[i - 1] < mini) mini = zkl.sa[i - 1];
            }
        }
    }
    printf("%lld\n", ans);
