/*
 ...
 HINTS about rotating & flipping:
	...
	As a axisymmetry block's flipping-rotating-status are included in rotating-status,	  and centrosymmery block can return to originnal form by rotate 180 degree,
...
int otm[MAXT + 1] = {4, 2, 1, 4, 8, 4, 8, 8, 8, 4, 8, 8, 67};
int sot[MAXT] = {0, 4, 6, 7, 11, 19, 23, 31, 39, 47, 51, 59};
void flp(int &x, int &y, int f)
{
	if (f) x = -x;
}
void rot(int &x, int &y, int b)
{
	while (b--)
	{
		swap(x, y);
...
vector<int> P;
bool CAI(int x, int y, int t, int f, int b)
{
	P.clear();
	Rep(i, 0, 5)
	{
